#!/usr/bin/env python3
"""
Mini Mind v7.1 - word-level context model with semantic topic state
Part of Templar Studios | GPL v3.0

Fixes over v7:
  - Load errors now print the actual exception (was silently failing)
  - Model pruning on save: contexts seen only once are discarded
  - context_categories capped at 8000 entries
  - START/END now correctly tagged as BOUNDARY (was "WORD")
  - categorize() now correctly matches START/END (was case-mismatch bug)
  - Default order reduced to 5 for ish/iOS memory safety
  - Vocab cap reduced to 3000
  - Save now handles OSError explicitly
  - Bigger safety guards on load for oversized files
  - Detokenizer closes dangling quote correctly
...
DEFAULT_MAX_ORDER = 5          # lowered from 7 for ish safety
DEFAULT_MIN_ORDER = 1
DEFAULT_LENGTH = 300
DEFAULT_TEMP = 0.8
...
# pruning (applied on save)
PRUNE_MIN_COUNT = 2           # discard contexts total count < this
CONTEXT_CAT_CAP = 8000        # max context_categories entries kept
VOCAB_CAP = 3000              # max vocab entries serialized

# load safety
MAX_JSON_BYTES = 40 * 1024 * 1024   # warn if model file > 40MB
...
def pos_tag(word: str) -> str:
    """POS tag. Boundary tokens get their own tag."""
...
def categorize(word: str) -> str:
    """Semantic category. Fixed: checks START/END with exact string."""
    if word in (START_TOKEN, END_TOKEN):
        return "BOUNDARY"
    if word in PUNCT_TOKENS:
        return "OTHER"
...
class MiniMind:
    ...
    def _prune(self) -> None:
        """Remove rarely-seen transitions to keep model compact."""
    ...
    def _predict(self, context: List[str], temperature: float,
        ...
        for order in range(self.max_order, self.min_order - 1, -1):
            ...
            # Category reranking from context
    ...
    def save(self) -> bool:
        ...
        try:
            ...
            if size > MAX_JSON_BYTES:
                print(f"[!] warning: model is {size/1024/1024:.1f} MB — may fail to load in ish", file=sys.stderr)
        except OSError:
            pass
    ...
    def load(self, name: Optional[str] = None) -> bool:
        ...
        try:
            size = self.model_file.stat().st_size
            if size > MAX_JSON_BYTES:
                print(f"[!] model too large ({size/1024/1024:.1f} MB) — retrain at lower order", file=sys.stderr)
                return False
        except OSError as e:
            print(f"[!] cannot stat model file: {e}", file=sys.stderr)
            return False
        ...
        try:
            ...
            print("[!] out of memory loading model — retrain at lower order", file=sys.stderr)
            return False
        ...
        try:
            ...
            for o_str, bucket in data.get("transitions", {}).items():
                ...
                self.transitions[order] = restored
            ...
            print("[!] out of memory rebuilding model — retrain at lower order", file=sys.stderr)
            return False
        except Exception as e:
            print(f"[!] model rebuild failed: {type(e).__name__}: {e}", file=sys.stderr)
            return False
...
def _detokenize_sentence(tokens: List[str]) -> str:
    ...
    if text and text[-1] not in ".!?":
        # don't add period right after an opening quote or bracket
        if text[-1] not in "\"')]":
            text += "."
    return text
...
def merge_all_models(max_order: int = DEFAULT_MAX_ORDER) -> MiniMind:
    ...
    for name in models:
        m = MiniMind()
        if not m.load(name):
            print(f"[!] skipping unloadable model: {name}", file=sys.stderr)
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="minimind",
        description="Mini Mind v7.1 - semantic topic model by Templar Studios",
    )
